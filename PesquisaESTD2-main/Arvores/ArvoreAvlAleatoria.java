...
public class ArvoreAvlAleatoria<T extends Comparable<T>> {
    ...
    public int caminho(Elemento e) {
        int contador = 1;
        while (e.elementopai != null) { 
            //  ANOTAÇÃO -> Enquanto não alcançamos a raiz faz-se
            contador++;
            e = e.elementopai;
        }
    ...
    public void percorrer(Consumer<T> callback) {  this.percorrer(raiz, callback);
    }

    public void percorrerInOrder(Consumer<T> callback) { this.percorrerInOrder(raiz, callback);
    }

    public void percorrerPosOrder(Consumer<T> callback) { this.percorrerPosOrder(raiz, callback);
    }
    ...
    public static void vetorAleatorio(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            do {
                UmNumero = ArvoreAvlAleatoria.gerarUmNumeroAleatorio(n);
